#!/usr/bin/env bash
# Seeds a test CMDB asset + incident directly on your ServiceNow PDI via the
# Table API, then exercises the TSM L1 Copilot's ServiceNow routes end-to-end
# against that real data. Run this from a machine with internet access
# (not the Claude sandbox — it can't reach service-now.com).
#
# Usage:
#   SN_URL=https://dev341015.service-now.com \
#   SN_USER=admin \
#   SN_PASS='your-pdi-password' \
#   TSM_URL=https://tsm-consultz.fly.dev \
#   ./test-servicenow-integration.sh
...
# Optional: only needed if you're hitting tsm-consultz.fly.dev directly and
# CF_GATE_SECRET is set as a Fly secret — the Cloudflare Worker normally
# injects this header, so raw curl to the .fly.dev origin needs it manually.
CF_GATE_HEADER=()
