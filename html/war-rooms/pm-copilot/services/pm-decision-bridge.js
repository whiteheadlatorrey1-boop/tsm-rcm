(function () {
  ...
  async function run(payload) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          'Decision engine HTTP ' + response.status
        );
      }
  ...
  function render(result) {
      ...
      .map(d => `
        <div style="padding:14px 0;border-top:1px solid rgba(255,255,255,.08)">
          <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
            ...
            <span>${esc(String(d.domain || '').replace(/_/g, ' '))}</span>
            <span style="margin-left:auto;font-weight:700">
              ${money(d.exposure)}
            </span>
          </div>

          <div style="margin-top:7px;font-weight:600">
            ${esc(d.finding)}
          </div>
          ...
          <div style="margin-top:5px;font-size:.88em;opacity:.65">
            Owner: ${esc(d.owner)}
            · Urgency: ${esc(d.urgency)}
            · Evidence: ${esc(d.entityId)}
          </div>
        </div>
      `)
      .join('');
    ...
    panel.innerHTML = `
      ...
      <p style="margin:0 0 16px;opacity:.8">
        ${esc(
          summary.headline ||
          'Management decisions generated from current PM evidence.'
        )}
      </p>
      ...
      ${
        rows ||
        '<div style="opacity:.65">No actionable decisions were generated from the current payload.</div>'
      }

      <div style="margin-top:14px;font-size:.78em;opacity:.55">
        Deterministic priority engine · Human approval required ·
