cat << 'EOF' > fix_inphusionsys_standalone.py
#!/usr/bin/env python3
import re
import os
...
window.loadInphusionAnomaly = function() {
  ...
  .catch(err => {
    console.warn("Backend API unavailable, using embedded standalone payload fallback:", err);
    if (targetBox && INPHUSION_PAYLOADS[vert]) {
      targetBox.value = INPHUSION_PAYLOADS[vert];
      targetBox.dispatchEvent(new Event("input", { bubbles: true }));
      targetBox.dispatchEvent(new Event("change", { bubbles: true }));
      alert("✓ Loaded [" + vert + "] Anomaly Payload (Offline Fallback Mode)!\\nAudit Hash: AUD-INPH-LOCAL-" + Math.floor(Math.random()*90000 + 10000));
    }
  });
};
</script>'''
...
print("✓ Embedded instant fallback payloads into front-end script!")
EOF

python3 fix_inphusionsys_standalone.py