#!/usr/bin/env bash
# activate-healthcare-enrichment.sh
# One-shot: insert the autoEnrichSentinel('healthcare') activation call into
# hc-main-strategist.html, right after the existing Sentinel relay write
...
#
# Usage:
#   ./activate-healthcare-enrichment.sh [path/to/hc-main-strategist.html]

set -euo pipefail

FILE="${1:-}"
if [ -z "$FILE" ]; then
  echo "No path given, searching repo for hc-main-strategist.html..."
  FILE=$(find . -name "hc-main-strategist.html" -not -path "*/node_modules/*" -not -path "*/backups/*" | head -1)
  if [ -z "$FILE" ]; then
    echo "ERROR: could not find hc-main-strategist.html. Pass the path explicitly:"
    echo "  ./activate-healthcare-enrichment.sh path/to/hc-main-strategist.html"
    exit 1
  fi
  echo "Found: $FILE"
fi
...
OLD = """            localStorage.setItem('TSM_HEALTHCARE_STRATEGIST_RELAY', JSON.stringify({
              generatedAt: new Date().toISOString(),
              anomalies: [hcAnomaly]
            }));
            window.dispatchEvent(new CustomEvent('TSM_SENTINEL_REFRESH'));"""

NEW = """            localStorage.setItem('TSM_HEALTHCARE_STRATEGIST_RELAY', JSON.stringify({
              generatedAt: new Date().toISOString(),
              anomalies: [hcAnomaly]
            }));
            if (window.TSMCapabilitySweep && typeof window.TSMCapabilitySweep.autoEnrichSentinel === 'function') {
              window.TSMCapabilitySweep.autoEnrichSentinel('healthcare');
            }
            window.dispatchEvent(new CustomEvent('TSM_SENTINEL_REFRESH'));"""
...
echo ""
echo "Verifying..."
grep -n "autoEnrichSentinel\|TSM_HEALTHCARE_STRATEGIST_RELAY\|TSM_SENTINEL_REFRESH" "$FILE"

echo ""
echo "Done. If the output above shows autoEnrichSentinel sitting between"
echo "the closing '}));' of the relay setItem and the TSM_SENTINEL_REFRESH dispatch, it's correct."