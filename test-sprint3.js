...
function testKnowledgeCopilot() {
  ...
  kc.upsertEntry('vendor', 'custom-onsite-visit', { title: 'Vendor Onsite Visit Required', steps: ['Schedule onsite technician', 'Confirm access/escort requirements'] });
  assert(kc.listEntries().length === before + 1, 'upsertEntry adds a new entry');
  assert(kc.lookup('vendor', 'custom-onsite-visit').title === 'Vendor Onsite Visit Required', 'new entry retrievable via lookup');
