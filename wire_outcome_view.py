#!/usr/bin/env python3
"""
Wires Roadmap #7 (TSMExecutiveOutcome) into html/healthcare/executive-portal.html.
Run from the repo root: python3 wire_outcome_view.py
Idempotent-ish: will error clearly if an anchor isn't found rather than
silently doing nothing or double-patching.
"""
import sys
...
def must_replace(content, old, new, label):
    ...
    if content.count(old) > 1:
        print(f"ABORT: anchor for [{label}] is not unique ({content.count(old)} matches) — "
              f"refusing to guess which one. No changes written.")
        sys.exit(1)
