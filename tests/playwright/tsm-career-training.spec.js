...
const PANELS = [
  ...
  'hc', 'crcr', 'finops', 'insurance', 'ahip',
  'realestate', 'mlo', 'bpo', 'msai', 'career', 'interview',
  'readiness', 'hitl',
  'deployment', 'earning', 'manager', 'governance', 'staffing'
];
// NOTE: 'rcm-director', 'staffing-coord', 'retention' removed -- no
// matching data-panel/#panel-* exists in tsm-career-training-platform.html
// as of this spec. Add them back once those panels actually ship.
...
test.describe('AB-100 Command Center · MSAI panel', () => {
  test('flashcard deck: flip, next, prev, shuffle', async ({ page }) => {
    ...
    await page.click('.fc-btn:has-text("NEXT")');
    await page.click('.fc-btn:has-text("PREV")');
    await page.click('.fc-btn:has-text("⇄")'); // shuffle is icon-only, no text label
