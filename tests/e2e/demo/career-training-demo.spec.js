...
test('Career Training Platform walkthrough', async ({ page }) => {
  await page.setViewportSize({ width: 1920, height: 1080 });
  test.setTimeout(120_000);

  const story = loadStory(path.join(__dirname, '../../../demo/career-training-demo.json'));
  const outDir = path.join(__dirname, 'screenshots', 'career-training');
