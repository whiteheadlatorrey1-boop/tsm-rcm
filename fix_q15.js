...
html = html.replace(/<script id="q15-interceptor">[\s\S]*?<\/script>/gi, "");
const script = `<script id="q15-interceptor">
(function() {
  ...
  function renderQ15Cards() {
    ...
    if (!grid) return;
    const playbooks = ["🔐 Account Lockout", "🔑 Password Reset", "🌐 VPN Connection", "📧 Outlook Auth", "🌁 Wi-Fi / Network", "💻 Boot Loop / Blue Screen", "🖥️ Remote Desktop", "🖨️ Printer Mapping", "📦 App Installation", "🛡️ MFA / Duo Reset", "💾 Disk Full", "📂 Share Permission", "🌐 Web Proxy", "📱 Mobile Sync", "🔒 BitLocker Recovery"];
    grid.innerHTML = playbooks.map(p => \`<div onclick="alert(&quot;Loaded Playbook: \${p}&quot;)" style="background:#1a1a1a; border:1px solid #333; padding:12px; border-radius:4px; cursor:pointer; color:#fff; font-size:12px; transition:0.2s;">\${p}</div>\`).join("");
  }
...
console.log("SUCCESS: Clean Q15 interceptor applied!");
