...
(function (global) {
  ...
  class CRMEngine {
    ...
    _entityDef(entityKey) {
      const SINGULAR_MAP = {
        leads: 'lead', contacts: 'contact', accounts: 'account',
        opportunities: 'opportunity', cases: 'case'
      };
      const singular = SINGULAR_MAP[entityKey] || entityKey.replace(/s$/, '');
    ...
    buildRelayPayload(aiText) {
      return {
        ...
        case_breaches: this.getSlaBreaches('cases'),
        opp_breaches: this.getSlaBreaches('opportunities'),
        opp_stage_distribution: this.getStageDistribution('opportunities'),
